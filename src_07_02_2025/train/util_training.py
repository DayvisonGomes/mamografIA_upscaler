...
def train_upsampler_ldm(
    ...
    early_stop = EarlyStopping(patience=21, verbose=True, path=os.path.join(model_save_path, "model_ldm_best_4_latent_spec_to_high_exp2.pth"))
    ...
    torch.save(raw_model.state_dict(), os.path.join(model_save_path, "model_ldm_4_latent_spec_to_high_exp2.pth"))
...
def train_epoch_upsampler_ldm(
    ...
    for step, x in pbar:
        ...
        with autocast(enabled=True):
            ...
            latent_model_input = torch.cat([noisy_latent, noisy_vmi_31, noisy_lumen_36], dim=1)
...
def eval_upsampler_ldm(
    ...
    for t in tqdm(scheduler.timesteps, ncols=110):
        with torch.no_grad():
            with autocast(enabled=True):
                latent_model_input = torch.cat([latents, noisy_vmi_31, noisy_lumen_36], dim=1)
