...
if __name__ == '__main__':
    ...
    train_loader, val_loader = get_upsampler_dataloader_combat(
    ...
    #images_vmi_31 = check_data["image_vmi_31"].to(device)
    #images_lumen_36 = check_data["image_lumen_36"].to(device)
    
    #images_concat = torch.cat([images, images_vmi_31, images_lumen_36], dim=1)
    with torch.no_grad():
        with autocast(enabled=True):
            z = stage1.encode_stage_2_inputs(images.to('cuda'))
