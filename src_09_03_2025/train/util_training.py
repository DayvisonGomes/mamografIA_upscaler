...
def train_aekl(
    ...
    early_stop = EarlyStopping(patience=7, verbose=True, path=os.path.join(model_save_path, "model_aekl_best_4_latent_new_dataset_toy_exp.pth"))
    ...
    torch.save(raw_model.state_dict(), os.path.join(model_save_path, "model_aekl_4_latent_new_dataset_toy_exp.pth"))
...
def train_epoch_aekl(
    ...
    for step, x in pbar:
        ...
        #images_vmi_31 = x["image_vmi_31"].to(device)
        #images_lumen_36 = x["image_lumen_36"].to(device)

        #images_concat = torch.cat([images, images_vmi_31, images_lumen_36], dim=1)
...
def train_upsampler_ldm(
    ...
    early_stop = EarlyStopping(patience=21, verbose=True, path=os.path.join(model_save_path, "model_ldm_best_4_latent_spec_to_high_exp_scaled_linear.pth"))
    ...
    #torch.save(raw_model.state_dict(), os.path.join(model_save_path, "model_ldm_4_latent_pulmao_new_input_sr_lossw_with_noise_exp7.pth"))
    torch.save(raw_model.state_dict(), os.path.join(model_save_path, "model_ldm_4_latent_spec_to_high_exp_scaled_linear.pth"))
...
def train_epoch_upsampler_ldm(
    ...
    for step, x in pbar:
        ...
        image_vmi_31 = x["image_vmi_31"].to(device)
        image_lumen_36 = x["image_lumen_36"].to(device)
        #image_iodine_38 = x["image_iodine_38"].to(device)
        ...
        #mask_low_res_image = F.interpolate(mask_tensor, size=(512, 512), mode='bilinear', align_corners=False).to(device)
        ...
        #images_concat = torch.cat([images, image_vmi_31, image_lumen_36], dim=1).to(device)
        ...
        with autocast(enabled=True):
            ...
            #noisy_vmi_31 = scheduler.add_noise(original_samples=image_vmi_31, noise=low_res_noise, timesteps=low_res_timesteps)
            #noisy_lumen_36 = scheduler.add_noise(original_samples=image_lumen_36, noise=low_res_noise, timesteps=low_res_timesteps)
            #noisy_iodine_38 = scheduler.add_noise(original_samples=image_iodine_38, noise=low_res_noise, timesteps=low_res_timesteps)
...
def eval_upsampler_ldm(
    ...
    image_vmi_31 = image_vmi_31[0].unsqueeze(0)
    image_lumen_36 = image_lumen_36[0].unsqueeze(0)
    #image_iodine_38 = image_iodine_38[0].unsqueeze(0)
    ...
    #noisy_vmi_31 = scheduler.add_noise(original_samples=image_vmi_31, 
                                      # noise=low_res_noise, timesteps=noise_level)
    #noisy_lumen_36 = scheduler.add_noise(original_samples=image_lumen_36, 
                                         #noise=low_res_noise, timesteps=noise_level)
    #noisy_iodine_38 = scheduler.add_noise(original_samples=image_iodine_38, 
                                         # noise=low_res_noise, timesteps=noise_level)
