...
if __name__ == '__main__':
    ...
    for batch in tqdm(eval_loader, ncols=110):
        ...
        with torch.no_grad():
            image = batch['image'].to(device)
            #images_vmi_31 = batch["image_vmi_31"].to(device)
            #images_lumen_36 = batch["image_lumen_36"].to(device)

            #images_concat = torch.cat([image, images_vmi_31, images_lumen_36], dim=1)
        ...
        for idx, image_batch in enumerate(batch['image']):
            ...
            img_name = batch['filename_image'][idx].split('.dcm')[0]
            path_img_recon_tiff = os.path.join(output_dir, f"{img_name}_recons.tiff")
            ...
            #img_recons = np.clip(img_recons, a_min=0.0, a_max=1.0)
