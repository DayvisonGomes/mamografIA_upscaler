...
from util_transformations import get_upsampler_dataloader, get_upsampler_dataloader_combat
...
if __name__ == '__main__':
    ...
    #     num_workers=4,
    # )
    train_loader, eval_loader = get_upsampler_dataloader_combat(
        batch_size=1,
    ...
    for batch in tqdm(eval_loader, ncols=110):
        ...
        #image_vmi_31 = batch["image_vmi_31"].to(device)
        #image_lumen_36 = batch["image_lumen_36"].to(device)
        #image_iodine_38 = batch["image_iodine_38"].to(device)
        ...
        #noisy_vmi_31 = scheduler.add_noise(original_samples=image_vmi_31, noise=low_res_noise, timesteps=torch.Tensor((noise_level,)).long().to(device))
        #noisy_lumen_36 = scheduler.add_noise(original_samples=image_lumen_36, noise=low_res_noise, timesteps=torch.Tensor((noise_level,)).long().to(device))
        #noisy_iodine_38 = scheduler.add_noise(original_samples=image_iodine_38, noise=low_res_noise, timesteps=torch.Tensor((noise_level,)).long().to(device))
        ...
        for idx, image_batch in enumerate(batch['image']):
            ...
            img_name = batch['filename_image'][0]
            img_low_name = batch['filename_low_res_image'][0]
            #img_vmi_31_name = batch['filename_image_vmi_31'][0]
            #img_lumen_36_name = batch['filename_image_lumen_36'][0]

            #path_img_up_tiff = os.path.join(output_dir, f"{img_name}_upscale.tiff")

            print(img_low_name)
            path_img_up_tiff = os.path.join(output_dir, f"{i}_{img_name}_generate.tiff")
            mask_path = img_name + '_mask.tiff'
            path_img_tiff = os.path.join(output_dir, f"{i}_{img_name}.tiff")
            path_img_tiff_low = os.path.join(output_dir, f"{i}_{img_low_name}_low.tiff")
            
            #path_img_tiff_vmi_31 = os.path.join(output_dir, f"{i}_{img_vmi_31_name}_vmi.tiff")
            #path_img_tiff_lumem_36 = os.path.join(output_dir, f"{i}_{img_lumen_36_name}_lumem.tiff")
            ...
            low_img = batch["low_res_image"][idx, 0].cpu().numpy()
            #img_vmi_31 = batch["image_vmi_31"][idx, 0].cpu().numpy()
            #img_lumen_36 = batch["image_lumen_36"][idx, 0].cpu().numpy()
            ...
            tifffile.imwrite(path_img_tiff_low, low_img)
            #tifffile.imwrite(path_img_tiff_vmi_31, img_vmi_31)
            #tifffile.imwrite(path_img_tiff_lumem_36, img_lumen_36)
