...
def creating_csvs(args):
    ...
    imagem, permitindo a aplicação de transformações nessas imagens.
    """
    path_data = r'D:\Users\bbruno\Data\ct_plaque\4Bruno\2024.07.17-plaque_S' 
    path_excel = r'D:\Users\bbruno\Data\ct_plaque\4Bruno\organized_table.xlsx'
    ...
    for sheet in xls.sheet_names:
        ...
        df = df.drop([11]).reset_index(drop=True)
        
        if sheet == 'GT_high':
            for _, row in df.iterrows():
                path = row['Root'].split('.\\')[-1]
                image_path = os.path.join(path_data, path)
                files = natsorted(os.listdir(image_path))[::-1]
                aux = []
                aux_2 = []
                ...
                for j, file in enumerate(files):
                    ...
                    if j % 2 == 0:
                        aux.append(os.path.join(image_path, file))
                        aux_2.append(path)
                
                for k in range(len(natsorted(aux))):
                    images_high.append({'image_high':aux[k], 'folder_high':aux_2[k]})
        ...
        if sheet == 'GT_low':
            for i, row in df.iterrows():
                ...
                for file in files:
                    images_low.append({"image_low": os.path.join(image_path, file),
                                "folder":path})
        ...
        if sheet == 'VMI_31':
            for i, row in df.iterrows():
                ...
                files = natsorted(os.listdir(image_path))
                for file in files:
                    images_vmi_31.append({"image_vmi_31": os.path.join(image_path, file),
                                "folder_vmi_31":path})
        ...
        if sheet == 'Lumen_36':
            for i, row in df.iterrows():
                ...
                files = natsorted(os.listdir(image_path))
                for file in files:
                    images_lumen_36.append({"image_lumen_36": os.path.join(image_path, file),
                                "folder_lumen_36":path})
        ...
        if sheet == 'Iodine_38':
            for i, row in df.iterrows():
                ...
                files = natsorted(os.listdir(image_path))
                for file in files:
                    images_iodine_38.append({"image_iodine_38": os.path.join(image_path, file),
                                "folder_iodine_38":path})
      
   
    #all_data.append({"image": path_file, "low_res_image": os.path.join(path_low_res_images, image_path)})
    df_high = pd.DataFrame(images_high)
    df_low = pd.DataFrame(images_low)
    df_vmi_31 = pd.DataFrame(images_vmi_31)
    df_lumen_36 = pd.DataFrame(images_lumen_36)
    df_iodine_38 = pd.DataFrame(images_iodine_38)

    df_all = pd.concat([df_high, df_low, df_vmi_31, df_lumen_36, df_iodine_38], axis=1)
 
    df_all = df_all.sample(frac=1, random_state=42).reset_index(drop=True)

    # usar spec_image_low pra gerar high image
    # usar low image pra gerar high_spec_image

    string_replace = r'D:\Users\bbruno\Data\ct_plaque\4Bruno\2024.07.17-plaque_S'

    df_all['image_high'] = df_all['image_high'].apply(lambda x: x.replace(string_replace, '/project/plaque_S'))
    df_all['image_high'] = df_all['image_high'].apply(lambda x: x.replace('\\', '/'))

    df_all['image_low'] = df_all['image_low'].apply(lambda x: x.replace(string_replace, '/project/plaque_S'))
    df_all['image_low'] = df_all['image_low'].apply(lambda x: x.replace('\\', '/'))

    df_all['image_vmi_31'] = df_all['image_vmi_31'].apply(lambda x: x.replace(string_replace, '/project/plaque_S'))
    df_all['image_vmi_31'] = df_all['image_vmi_31'].apply(lambda x: x.replace('\\', '/'))

    df_all['image_lumen_36'] = df_all['image_lumen_36'].apply(lambda x: x.replace(string_replace, '/project/plaque_S'))
    df_all['image_lumen_36'] = df_all['image_lumen_36'].apply(lambda x: x.replace('\\', '/'))

    df_all['image_iodine_38'] = df_all['image_iodine_38'].apply(lambda x: x.replace(string_replace, '/project/plaque_S'))
    df_all['image_iodine_38'] = df_all['image_iodine_38'].apply(lambda x: x.replace('\\', '/'))
    ...
    folders = df_all['folder_high'].unique()
    train_folders, test_val_folders = train_test_split(folders, test_size=0.1, random_state=42)
    test_folders, val_folders = train_test_split(test_val_folders, test_size=0.5, random_state=42)

    train_data_list = df_all[df_all['folder_high'].isin(train_folders)].reset_index(drop=True)
    test_data_list = df_all[df_all['folder_high'].isin(test_folders)].reset_index(drop=True)
    val_data_list = df_all[df_all['folder_high'].isin(val_folders)].reset_index(drop=True)

    #train_data_list, test_val_data = train_test_split(df_final, test_size=0.2, random_state=42)
    #val_data_list, test_data_list = train_test_split(test_val_data, test_size=0.5, random_state=42)
    #train_data_list = pd.concat([train_data_list, test_data_list])
